...
export const renderIndicatorChapter = props => {
  ...
  const isHiding = hideNextAfterSort(head, depth, coords);
  console.log(`hide next After Sort? ${isHiding}`);
...
const hideNextAfterSort = (head, targetDepth, coords) => {
  let res = head;
  for (let i = 2; i < targetDepth - 1; ++i) {
    res = res.child[coords[`d${i}`]];
  }

  return res?.child[coords[`d${targetDepth}`] + 1].deck?.isHide;
};
...
//   return nxtCard !== undefined;
// };

// // const hasNextD1 = (head, targetDepth, coords) => {
// //   let res = head;
// //   for (let i = 2; i < targetDepth; ++i) {
// //     res = res.child[coords[`d${i}`]];
// //   }
...
//     res = res.child[coords[`d${i}`]];
//   }
//   const nxtDepthCards = res.child[`d${targetDepth}`].deck;
//   return nxtDepthCards.length > 0;
// };

// // const hasNextDepthD1 = (head, targetDepth, coords) => {
// //   let res = head;
// //   for (let i = 2; i < targetDepth + 1; ++i) {
// //     res = res.child[coords[`d${i}`]];
// //   }
